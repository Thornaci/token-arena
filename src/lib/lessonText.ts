...
type MessageFn = (params?: Record<string, never>, options?: { locale?: Locale }) => string;
 ...
 */
export function lessonText(key: string, locale?: Locale): string {
  const candidate = (m as Record<string, unknown>)[key];
  if (typeof candidate !== 'function') return key;
  return (candidate as MessageFn)({}, locale ? { locale } : undefined);
}
